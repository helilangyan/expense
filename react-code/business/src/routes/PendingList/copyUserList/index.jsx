...
class approvedList extends Component {
            ...
            columns: [
                {
                    ...
                    render: (text, record) => {
                        if (record.bpmKey) {
                            if (record.bpmKey.split('_')[2] == '1') {
                                return (
                                    <span>报销申请</span>
                                )
                            } else if (record.bpmKey.split('_')[2] == '2') {
                                return (
                                    <span>出差申请</span>
                                )
                            } else if (record.bpmKey.split('_')[2] == '3') {
                                return (
                                    <span>借款申请</span>
                                )
                            } else {
                ...
                },
                {
                    ...
                    render: (text, record) => (
                        <span>
                            {(new Date(text)).getFullYear()+'-'+(new Date(text).getMonth() + 1 < 10 ? '0' + (new Date(text).getMonth() + 1) : new Date(text).getMonth() + 1) + '-'+(new Date(text).getDate() < 10 ? '0' + (new Date(text).getDate()) : new Date(text).getDate())}
                        </span>
                    ),
                }, {
                    title: '当前处理人',
    ...
    getData(current) {
        this.post(`/detail/apply-copy/list`,
            {
    ...
    seeBpmn = (data) => {
        data.businessKey=data.businessId
        data.key=data.bpmKey
