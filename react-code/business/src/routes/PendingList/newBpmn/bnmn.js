...
class bpmn extends Component {
    async componentDidMount(){
        ...
        const that=this;
        let xmlText='';
        let key=this.props.checkData.key;
        await this.get('/bpm/design/design/getDefinitionXMLDeployId/'+key.split('_')[1]+'/'+key.split('_')[2]).then(res=>{
            if (res.code==1){
                xmlText=res.data;
                this.post('/bpm/use/history/pic',{businessKey:this.props.checkData.businessKey}).then(res=>{
                    if (res.code==1){
                        registryData=res.data.highLightPoint;
    ...
    initBpmn(xmlText){
        ...
        viewer.importXML(xmlText, function(err) {
            ...
            const _modeling = viewer.get('modeling');
            const elementRegistry = viewer.get('elementRegistry');
            let regData=[];
            registryData.forEach(item=>{
                regData.push(elementRegistry.get(item))
            })
            console.log(regData);
            console.log(_modeling);
            _modeling.setColor(regData, {
                stroke: 'green',
                fill: 'yellow'
            });
        });
    ...
    render() {
        return (
            <div>
                <div>
                    <div id="canvas" style={{height: '70vh',marginTop:'50px'}} onClick={this.bpmnClick}/>
