...
class tripCheck extends Component {
  ...
  // 限制输入数字和小数点
  numLimit = (num) => {
    return num = num ? num.replace(/[^\d.]/g, '') : ''
  }
  // 格式化时间
          ...
          <Row>
            <Col span={12}>
              <Form.Item label='出发日期' name='startTime' rules={[{ required: true, validator: validateStartDate }]}>
                <DatePicker placeholder='请输入出发日期' style={{ width: '100%' }} disabledDate={disabledDate} onOpenChange={dateMessage}/>
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item label='到达日期' name='endTime' rules={[{ required: true, validator: validateEndDate }]}>
                <DatePicker placeholder='请输入到达日期' style={{ width: '100%' }} disabledDate={disabledDate} onOpenChange={dateMessage}/>
              </Form.Item>
            </Col>
            <Col span={12}>
