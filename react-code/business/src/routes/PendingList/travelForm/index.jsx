...
export default class TravelForm extends Component {
	...
	setDefaultData = () => {
        if (this.props.defaultData) {
			const { applyTravelEntity, detailList, userList } = this.props.defaultData
    ...
    let data = {...this.state.formData,...v}
		this.setState({ formData: data, limitTime: time })
	}
	checkPromise = (e) => {
	...
	render() {
		const { data, addlist, columns, totalCount, totalDays, tripData, newItem,history,
			namelist, userlist, valiText, valiStatus, tripStatus, tripText, limitTime } = this.state
		...
		return (
			<div>
				<Row>
					<Form style={styles.form} {...formItemLayout} ref={this.formRef} onValuesChange={this.onChange}>
						<Form.Item label='申请名称' name='applyName' rules={[{ required: true, message: '请输入申请名称!' }]}>
							<Input placeholder='请输入申请名称' name='applyName' readOnly />
						</Form.Item>
						...
						<Form.Item label='说明' name='remark'>
							<Input.TextArea readOnly placeholder='请输入说明' />
						</Form.Item>
						{
							history.length !== 0 ?
				...
				>
					{
						newItem == 0 ?
							<TripCheck addTrip={this.addTrip} onCancel={this.handleCancel} limitTime={limitTime} /> :
