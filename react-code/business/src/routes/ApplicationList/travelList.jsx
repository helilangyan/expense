...
export default class TravelList extends Component {
    ...
    getData = (searchData) => {
        ...
        this.post(`/detail/apply-travel/list`, {
            limit: pageSize,
            page: pageNum,
            // userId: data.userId,
    ...
    // 打开费用详情
    viewDetail = (e) => {
        this.get(`/detail/apply-travel/` + e.id).then(res => {
            if (res.code === 1) {
                let obj = res.data
                obj.status= e.status
                this.setState({
    ...
    onSubmits = (subData, type) => {
        this.props.onSubmit(subData, type);
        this.onCancel();
    }
    onDelete = (id) => {
        this.del(`/detail/apply-travel/delete/` + id).then(res => {
            if (res.code === 1) {
    ...
    formatList = (datalist) => {
        ...
        // 按时间排二级列表
        datalist.map(item => {
            month.push(item.createTime.slice(5, 7))
            month = dedupe(month)
            month.sort((a, b) => {
                return a - b
            }); //月份排序
        ...
        Object.keys(listObj).forEach(function (key) {
            childData = {
                name: listObj[key][0].createTime.slice(0, 7),
                child: listObj[key]
            }
        ...
        return (
            <div className="contentBox">
                                ...
                                renderItem={item => <List.Item>
                                    <div className="applyItem" key={item.id} style={styles.applyItem}>
                                        <Row type="flex" justify="center" align="middle">
                                            ...
                                            <Col span={6}>
                                                <p className="con">日期：<span>{item.createTime}</span></p>
                                            </Col>
                                            <Col span={5}>
                                                {
                                                    item.status == 2 ?
                                                        <Button type="primary" onClick={() => this.viewDetail(item)}
                                                                className="btn-success color-btn">已启动</Button> :
                                                        item.status == 1 ?
                                                            <Button type="primary" onClick={() => this.viewDetail(item)}
                                                                    className="btn-success color-btn">审批中</Button> :
                                                            item.status == 3 ?
                                                                <Button type="primary" onClick={() => this.viewDetail(item)}
                                                                        className="btn-danger color-btn">已拒绝</Button> :
                                                                item.status == 9 ?
                                                                    <Button type="primary" onClick={() => this.viewDetail(item)}
                                                                            className="btn-purple color-btn">已通过</Button> :
                                                                    <Button type="primary"
                                                                            onClick={() => this.viewDetail(item)}>打开</Button>
                                                }
                                            </Col>
                                            <Col span={1} style={{textAlign: 'right'}}>
                                                {
                                                    item.status == null ?
                                                        <Popconfirm placement="left" title={confirmText}
                                                                    onConfirm={() => this.onDelete(item.id)}
                                                                    okText="Yes" cancelText="No">
                                                            <Button type="text"
                                                                    className="del"><DeleteOutlined/></Button>
                                                        </Popconfirm> : ''
                                                }
                                            </Col>
                                        </Row>
                ...
                <Modal
                    ...
                    destroyOnClose={true}
                >
                    <TravelForm onSubmit={this.onSubmits} onCancel={this.onCancel} defaultData={defaultData}/>
                </Modal>
            </div>
        )
    }
