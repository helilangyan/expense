...
import ApplyForm from './ApplicationModal/applyForm/index'
...
export default class ApplyList extends Component {
  ...
  getData = (searchData) => {
    ...
    this.post("/detail/apply-expense/list", {
      limit: pageSize,
      page: pageNum,
      // userId: data.userId,
  ...
  // 打开费用详情
  viewDetail = (e) => {
    this.get(`/detail/apply-expense/` + e.id).then(res => {
      if (res.code === 1) {
        let obj = res.data
        obj.status = e.status
        this.setState({
  ...
  onDelete = (id) => {
    this.del("/detail/apply-expense/delete/" + id).then(res => {
      if (res.code === 1) {
  ...
  render() {
    const { editModal, datalist, defaultData, pageNum, pageSize, total } = this.state
    ...
    return (
      <div className="contentBox">
                ...
                renderItem={item => <List.Item>
                  <div className="applyItem" key={item.id} style={styles.applyItem}>
                    <Row type="flex" justify="center" align="middle">
                      ...
                      {/* <Col span={3}>
                        <AccountBookOutlined style={styles.icon} /><span style={styles.num}>{'×3'}</span>
                      </Col> */}
                      <Col span={5}>
        ...
        <Modal
          ...
          destroyOnClose={true}
        >
          <ApplyForm onSubmit={this.onSubmit} onCancel={this.onCancel} defaultData={defaultData} />
        </Modal>
      </div>
    )
  }
