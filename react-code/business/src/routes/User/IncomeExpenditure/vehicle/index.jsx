...
export default class Vehicle extends Component {
  ...
  getData(current) {
    this.post(`/enterprise/strategy-vehicle/list`,
      {
  ...
  handleStatus = (e) => {
    ...
    this.post(`/enterprise/strategy-vehicle/update-status`, status, 'application/json').then(res => {
      if (res.code == 1) {
        this.handleCancel();
  ...
  submitInput = (data) => {
    const that = this;
    this.post(`/enterprise/strategy-vehicle/insert`, data, 'application/json').then(res => {
      if (res.code == 1) {
        that.handleCancel();
  ...
  deleteClick = (data) => {
    ...
    confirm({
      ...
      cancelText: '取消',
      onOk() {
        that.del(`/enterprise/strategy-vehicle/del/` + data.id).then(res => {
          if (res.code == 1) {
